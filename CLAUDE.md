...
1. **Data consistency fix** — front page Yesterday panel and /performance-html show different W/L numbers for same date. Root cause: picks graded against 6am DB snapshot vs picks regenerated by afternoon refresh. Fix grader to match displayed picks.
2. **Game picks at 6am independent of lineups** — ML/RL/total picks should publish immediately after 6am pipeline. Lineup confirmation only needed for props, not game picks.
3. **Projected props** — publish props early for everyday regulars flagged as PROJECTED, update to CONFIRMED or drop when official lineup posts. Mirrors sportsbook pre-lineup prop behavior.
4. **Pick card narrative** — 2-3 sentence plain-English explanation on each card synthesizing top contributing factors (pitcher mismatch, rest edge, market signal, etc.)
5. **Thematic parlays** — group parlay legs by shared thesis (Pitching Mismatch, Home Dog Value, Rest Advantage, Fresh Pen, Sharp Action) instead of flat confidence stacking.
6. **Fix Kalshi market matching** — scraper logs "Parsed 0 unique game markets"; all picks currently NEUTRAL on market signal.
7. **TBD starter suppression** — auto-downgrade picks one tier when either SP is TBD (model uses league-average ERA = fake confidence).
8. **Run line edge threshold** — RL picks require model confidence >70% before publishing (vs 60% for ML). Current RL ROI: -52.3%.
